...
def apply_chart_style(fig, x_series=None, max_val=None, text_size=20, x_size=19, y_size=17, title_size=22, is_group=False, force_bar_width=False):
    ...
    if x_series is not None:
        ...
        # 막대 두께 완전 고정 로직
        if not is_group:
            # 주차별 해지 차트 등 얇게 고정해야 하는 경우 강제 두께 설정
            if force_bar_width:
                fig.update_traces(width=0.4) # 모든 막대의 픽셀/비율 두께를 동일하게 고정
            else:
            ...
            for idx, week_name in enumerate(weeks):
                ...
                with target_col:
                    ...
                    if not df_week.empty:
                        ...
                        # 여기도 X축 카테고리가 적을 수 있으므로 강제 고정 옵션 추가
                    ...
                    for idx, week_name in enumerate(weeks):
                        ...
                        with target_col:
                            ...
                            if not df_cw.empty and '해지사유' in df_cw.columns:
                                ...
                                # 막대그래프 두께를 3주차와 동일하게 얇게 고정 (force_bar_width=True)
